...
- 敌人与节奏：每关敌人数量随关卡递增，最多同时 3 台在场；敌方出生前 1 秒会在落点出现预警光环；靠近玩家会追击，避开边界和墙体，出生附带短暂无敌盾。通关后会弹出提示，确认后进入下一关。
- 生命与复活：每关开始时生命补满为 `2 + 当前关数`（第 1 关 3 条）；被击中会短暂等待复活；生命耗尽时弹出“重试”对话框，确认后从当前关重开。
...
- 导弹（missile）：装填 4 发追踪导弹，会自动锁定附近敌方单位；若在有效时间内未命中则自爆。
